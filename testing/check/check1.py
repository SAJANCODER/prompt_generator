...
class MasterPromptEngineer:
    ...
    def _load_data(self) -> None:
        """Load all data including authentication information."""
        try:
            ...
            if os.path.exists("../../user/auth_tokens.json"):
                with open("../../user/auth_tokens.json", "r", encoding="utf-8") as f:
    ...
    def _save_data(self) -> None:
        """Save all data including authentication information."""
        try:
            ...
            # Save auth tokens
            with open("../../user/auth_tokens.json", "w", encoding="utf-8") as f:
                json.dump(self.auth_tokens, f, indent=2, ensure_ascii=False)
    ...
    def _clean_expired_tokens(self) -> None:
        ...
        now = datetime.now()
        expired = [email for email, token_data in self.auth_tokens.items()
                   if datetime.fromisoformat(token_data["expiry"]) < now]
        for email in expired:
            del self.auth_tokens[email]

    def _send_auth_email(self, email: str, token: str) -> bool:
        """Send authentication email with login token."""
        try:
            msg = MIMEText(f"""
            Your Master Prompt Engineer login token is:

            {token}

            This token will expire in {self.email_config['token_expiry_hours']} hours.
            """)
            msg['Subject'] = 'Your Master Prompt Engineer Login Token'
            ...
            warnings.warn(f"Failed to send auth email: {str(e)}")
            return False
    ...
    def request_login_token(self, email: str) -> bool:
        """Request a login token via email."""
        if email not in self.user_profiles:
            return False  # Don't reveal whether user exists

        token = self._generate_token()
        expiry = datetime.now() + timedelta(hours=self.email_config['token_expiry_hours'])
        ...
        return self._send_auth_email(email, token)

    def verify_token(self, email: str, token: str) -> bool:
        """Verify a login token."""
        if email not in self.auth_tokens:
            return False

        token_data = self.auth_tokens[email]
        if datetime.fromisoformat(token_data["expiry"]) < datetime.now():
            del self.auth_tokens[email]
            return False

        if secrets.compare_digest(token_data["token"], token):
            # Update user profile on successful login
            self.user_profiles[email]["last_login"] = datetime.now().isoformat()
            self._save_data()
            del self.auth_tokens[email]  # Token is single-use
            return True
        return False
    ...
    def login_with_password(self, email: str, password: str) -> bool:
        ...
        if not self._verify_password(user.get("password_hash"), password):
            return False
    ...
    def interact(self):
        """Enhanced interaction loop with authentication."""
        print("\nWelcome to Master Prompt Engineering System!")
        print("1. Register\n2. Login with Email Token\n3. Login with Password\n4. Continue as Guest")
        ...
        if choice == "1":
            ...
            if self.register_user(email, password):
                ...
                print("Registration failed (email may be taken or invalid).")
            return self.interact()

        elif choice == "2":
            email = input("Enter your email: ").strip()
            if self.request_login_token(email):
                print("Login token sent to your email. Check your inbox.")
                token = input("Enter the token you received: ").strip()
                if self.verify_token(email, token):
                    print("Authentication successful!")
                    return self._authenticated_interact(email)
                else:
                    print("Invalid or expired token.")
            else:
                print("Failed to send token. Please try again.")
            return self.interact()

        elif choice == "3":
            email = input("Enter your email: ").strip()
            password = input("Enter your password: ").strip()
            if self.login_with_password(email, password):
            ...
            print("Continuing as guest (limited functionality).")
            user_id = "guest_" + str(random.randint(1000, 9999))
            return self._authenticated_interact(user_id)

        else:
            print("Invalid choice.")
            return self.interact()

    def _authenticated_interact(self, user_id: str):
        """Interaction loop for authenticated users."""
        print(f"\nWelcome, {user_id}!")
        ...
        while True:
            try:
                ...
                # ... (rest of existing interact logic remains the same)
